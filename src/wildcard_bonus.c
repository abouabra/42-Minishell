...
/*   Updated: 2023/08/19 10:11:53 by ykhayri          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	many_wild_loop(int n[5], char *av, struct dirent *read, t_chars *chars)
{
	...
	else if (chars->strings[tmp] && ((n[j] > 0 && n[j] < n[i] - 1)
...
void	many_wild(int n[5], char *av, struct dirent *read, t_chars *chars)
{
	...
	while (++n[j] < n[i])
		many_wild_loop(n, av, read, chars);
	if (n[valid] == n[i])
		chars->strings[final] = ft_strjoin(chars->strings[final],
				ft_strjoin(read->d_name, " "));
}
